...
def parse_api_response(response):
    ...
    nft_achievements = generate_description_list(response["story"]["nftAchievements"])
    defi_achievements = generate_description_list(response["story"]["deFiAchievements"])
    community_achievements = generate_description_list(response["story"]["communityAchievements"])
    vibe_achievements = generate_info_list(response["story"]["vibeAchievements"])
...
class ChatBot(discord.Client):
    ...
    async def on_message(self, message):
        ...
        if message.content.startswith('!passport '):
            ...
            # pprint(sample_passport["items"])
            
            # parsed_data = parse_passport(sample_passport)
